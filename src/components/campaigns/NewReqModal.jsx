...
function Stepper({ value, onChange, min = 1 }) {
  const b = "flex size-[28px] items-center justify-center rounded-[8px] border border-line bg-glass text-[14px] text-ink shadow-sm transition-all duration-150 hover:-translate-y-px hover:shadow-md active:translate-y-0";
...
function DropdownSelect({ options: io, value, onChange, placeholder, allowNew }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(!open)} className={`flex w-full items-center justify-between rounded-[10px] border border-line bg-glass px-3 py-2 text-left text-[13px] backdrop-blur-sm transition-all duration-200 ${value ? "text-ink" : "text-mute"}`}><span className="truncate">{value || placeholder}</span><span className="text-[10px] text-mute">▾</span></button>
      {open && (
        <div className="absolute inset-x-0 top-[calc(100%+5px)] z-[60] max-h-[170px] overflow-y-auto rounded-[12px] border border-line bg-glass-strong py-1 shadow-[0_16px_40px_rgba(25,22,17,0.14)] backdrop-blur-xl">
          ...
          {allowNew && (<div className="flex gap-1 border-t border-line px-2.5 py-1.5"><input value={nv} onChange={e => setNv(e.target.value)} placeholder="Add new..." className="flex-1 rounded-lg border border-line bg-glass px-2 py-1 text-[12px] text-ink outline-none"/><button onClick={() => { if (nv.trim()) { setOpts(p => [...p, nv.trim()]); onChange(nv.trim()); setNv(""); setOpen(false); } }} className="rounded-lg bg-accent px-2.5 py-1 text-[11px] font-semibold text-white shadow-sm">Add</button></div>)}
        </div>
      )}
    </div>
  );
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 overflow-y-auto py-1">
        {msgs.map((m, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, ease: [0.16, 1, 0.3, 1] }}
            ...
            <div className={`max-w-[85%] rounded-[14px] border px-3.5 py-2.5 text-[13px] leading-normal text-ink shadow-sm ${m.role === "user" ? "border-accent/[0.1] bg-accent/[0.05]" : "border-line bg-glass"}`}>{m.content}</div>
          </motion.div>
        ))}
        <div ref={endRef}/>
      </div>
            ...
            <div>
              ...
              {curStep.allowCustom && (
                <div className="mt-2 flex gap-1.5">
                  <input value={customInput} onChange={e => setCustomInput(e.target.value)} placeholder="Or type your own..." className="flex-1 rounded-[10px] border border-line bg-glass px-3 py-1.5 text-[12px] text-ink outline-none"/>
      ...
      {isDone && (
        <div className="border-t border-line pt-2.5">
          ...
          <div className="rounded-[14px] border border-green/[0.15] bg-glass px-3 py-2.5 text-[12px] leading-relaxed text-ink shadow-sm backdrop-blur-sm">
...
export default function NewReqModal({ onClose, onSubmit }) {
  ...
  const modeCard = "flex-1 cursor-pointer rounded-[18px] border border-line bg-glass px-4 py-6 text-center shadow-sm backdrop-blur-sm transition-all duration-200 ease-out hover:-translate-y-[3px] hover:border-accent/25 hover:shadow-[0_14px_32px_rgba(25,22,17,0.08)]";
          ...
          {mode === "form" && (<>
            ...
            <div className="mb-3"><label className={labelCls}>Budget (Lakhs ₹) — max 1.5 Cr</label>
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex items-center gap-1 rounded-lg border border-line bg-glass px-1.5 py-1">
            ...
            {svc === "Influencer Marketing" && (<>
              ...
              <div className="mb-3 rounded-[14px] border border-line bg-glass px-3.5 py-3 shadow-sm backdrop-blur-sm">
              ...
              <div className="mb-3"><label className={labelCls}>Product & Volume</label><div className="flex flex-col gap-1.5">{IM_PRODUCTS.map(p => { const a = products.includes(p.id); return (<div key={p.id} className="flex items-center gap-1.5"><button onClick={() => toggleProduct(p.id)} className={`flex-1 rounded-[10px] border px-3 py-1.5 text-left text-[11px] font-medium transition-all duration-150 ${a ? chipOn : chipOff}`}>{p.label}</button>
                {a && <input type="number" min={1} value={productVols[p.id] || 1} onChange={e => setProductVols({ ...productVols, [p.id]: Math.max(1, parseInt(e.target.value) || 1) })} className="w-[42px] rounded-lg border border-line bg-glass p-1 text-center text-[12px] text-ink outline-none"/>}</div>); })}</div></div>
